...
import MechanicDashboard from './components/MachineManagement/MechanicDashboard';
...
const App: React.FC = () => {
        ...
        const loadData = async () => {
            try {
                const [
                    ...
                    fetchTable<StockItem>('stock_items').catch(() => []),
                    fetchTable<ConferenceData>('conferences').catch(() => []),
                    fetchTable<TransferRecord>('transfers', { orderBy: 'created_at', ascending: false, limit: 3000 }).catch(() => []),
                    fetchTable<ProductionOrderData>('production_orders', { orderBy: 'created_at', ascending: false, limit: 3000 }).catch(() => []),
                    fetchTable<FinishedProductItem>('finished_goods', { orderBy: 'created_at', ascending: false, limit: 3000 }).catch(() => []),
                    fetchTable<PontaItem>('pontas_stock', { orderBy: 'created_at', ascending: false, limit: 2000 }).catch(() => []),
                    fetchTable<FinishedGoodsTransferRecord>('finished_goods_transfers', { orderBy: 'created_at', ascending: false, limit: 3000 }).catch(() => []),
                    fetchTable<PartsRequest>('parts_requests', { orderBy: 'created_at', ascending: false }).catch(() => []),
                    fetchTable<ShiftReport>('shift_reports', { orderBy: 'created_at', ascending: false, limit: 3000 }).catch(() => []),
                    fetchTable<ProductionRecord>('production_records', { orderBy: 'created_at', ascending: false, limit: 4000 }).catch(() => []),
                    ...
                    fetchTable<UserAccessLog>('user_access_logs', { orderBy: 'created_at', ascending: false, limit: 1000 }).catch(() => []),
    ...
    const renderPage = () => {
        ...
        switch (page) {
            ...
            case 'conferenceHistory': return <StockControl stock={stock} conferences={conferences} transfers={transfers} setPage={setPage} addConference={addConference} deleteStockItem={deleteStockItem} updateStockItem={(item) => updateStockItem(item.id, item)} createTransfer={createTransfer} editConference={editConference} deleteConference={deleteConference} productionOrders={productionOrders} initialView="history" gauges={gauges} currentUser={currentUser} activeBrandingPartner={activeBrandingPartner} />;
            case 'inventoryConference': return <InventoryConference activeBrandingPartner={activeBrandingPartner} />;
            ...
            case 'mechanicDashboard': return <MechanicDashboard activeBrandingPartner={activeBrandingPartner} />;
    ...
    return (
        <div className="app-container">
            ...
            <main className="main-content">
                ...
                <div className={currentUser && page !== 'login' ? 'p-4 print:p-0' : ''}>
                    {renderPage()}
                </div>
            </main>
        </div>
